...
import src.app_functions_sdk_py.constants
...
class TestEnvironment(unittest.TestCase):
    ...
    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_USE_REGISTRY: 'true'})
    def test_use_registry(self):
        logger = MagicMock()
        result, override = environment.use_registry(logger)
        self.assertTrue(result)
        self.assertTrue(override)
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_USE_REGISTRY, None)
        result, override = environment.use_registry(logger)
        self.assertFalse(result)
        self.assertFalse(override)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_SECURITY_SECRET_STORE: 'TRUE'})
    def test_use_security_secret_store(self):
        logger = MagicMock()
        result = environment.use_security_secret_store(logger)
        self.assertTrue(result)
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_SECURITY_SECRET_STORE, None)
        result = environment.use_security_secret_store(logger)
        self.assertFalse(result)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_COMMON_CONFIG: test_config_file})
    def test_get_common_config(self):
        ...
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_COMMON_CONFIG, None)
        result = environment.get_common_config_file_name(logger, default_config_file)
        self.assertEqual(result, default_config_file)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_FILE: test_config_file})
    def test_get_config_file(self):
        logger = MagicMock()
        result = environment.get_config_file_name(logger, default_config_file)
        self.assertEqual(result, test_config_file)
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_FILE, None)
        result = environment.get_config_file_name(logger, default_config_file)
        self.assertEqual(result, default_config_file)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_PROFILE: test_dir})
    def test_get_profile_dir(self):
        ...
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_PROFILE, None)
        result = environment.get_profile_directory(logger, default_dir)
        self.assertEqual(result, f"{default_dir}/")

    @patch.dict('os.environ',
                {src.app_functions_sdk_py.constants.ENV_KEY_REMOTE_SERVICE_HOSTS: ','.join(test_remote_hosts)})
    def test_get_remote_service_hosts(self):
        ...
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_REMOTE_SERVICE_HOSTS, None)
        result = environment.get_remote_service_hosts(logger, default_remote_hosts)
        self.assertListEqual(result, default_remote_hosts)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_DIR: test_dir})
    def test_get_config_dir(self):
        logger = MagicMock()
        result = environment.get_config_directory(logger, default_dir)
        self.assertEqual(result, test_dir)
        os.environ[src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_DIR] = ""
    ...
    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_FILE_URI_TIMEOUT: test_timeout})
    def test_get_request_timeout(self):
        ...
        os.environ[src.app_functions_sdk_py.constants.ENV_KEY_FILE_URI_TIMEOUT] = ""
        result = environment.get_request_timeout(logger, "")
        self.assertEqual(result, environment.DEFAULT_FILE_URI_TIMEOUT)
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_FILE_URI_TIMEOUT, None)
        result = environment.get_request_timeout(logger, default_timeout)
        self.assertEqual(result, default_timeout)

    @patch.dict('os.environ', {src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_PROVIDER_URL: test_provider})
    def test_get_request_timeout(self):
        logger = MagicMock()
        result = environment.get_config_provider_url(logger, default_provider)
        self.assertEqual(result, test_provider)
        os.environ[src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_PROVIDER_URL] = environment.NO_CONFIG_PROVIDER
        result = environment.get_config_provider_url(logger, default_provider)
        self.assertEqual(result, "")
        os.environ.pop(src.app_functions_sdk_py.constants.ENV_KEY_CONFIG_PROVIDER_URL, None)
        result = environment.get_config_provider_url(logger, default_provider)
